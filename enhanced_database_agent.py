...
Агент получает конфигурацию из базы данных PostgreSQL
Полная совместимость с пользовательскими шаблонами
"""
...
class PluginManager:
    ...
    def load_plugin(self, plugin_path: Path):
        ...
        if spec and spec.loader:
            ...
            # Проверяем наличие требуемых функций
...
class DatabaseConfigManager:
    ...
    def load_agent_config(self, template_id: int, agent_id: str = None) -> Optional[Dict[str, Any]]:
        ...
        # Базовая конфигурация из шаблона (полная совместимость с JSON форматом)
        config = {
...
class DatabaseActivityAgent:
    ...
    def convert_db_app_config(self, db_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Поддерживаем несколько форматов конфигурации
        
        # Формат 1: с полным execution блоком
        if 'execution' in db_config:
            execution = db_config['execution']
            activities_db = db_config.get('activities', [])
            
            # Конвертируем активности
            activities = []
        ...
        # Формат 2: прямой формат агента (для обратной совместимости)
        elif 'open' in db_config and 'activities' in db_config:
            return db_config
        
        # Формат 3: упрощенный формат
        else:
            return {
    ...
    def run(self):
        ...
        logging.info(f"Starting Enhanced Database Activity Agent")
