...
class DatabaseActivityAgent:
    ...
    def run(self):
        ...
        logging.info(f"Starting Enhanced Updatable Agent with continuous loop")
        ...
        try:
            while True:
                try:
                    # Проверяем обновления через мьютекс систему каждые 5 минут
                    if hasattr(self, '_last_update_check'):
                        if time.time() - self._last_update_check >= 300:  # 5 минут
                            if self.update_manager and self.update_manager._check_for_updates():
                                logging.info("New version detected, shutting down for update...")
                                self.shutdown()
                                sys.exit(0)
                    ...
                    # Выполняем задачи агента
                    self.perform_agent_tasks()
                    
                    # Короткий интервал для более частой проверки состояния
                    ...
                    self.db_manager.update_agent_status(self.agent_id, "error", f"Main loop error: {str(e)}")
                    # Продолжаем работу после ошибки
                    time.sleep(60)
        finally:
            self.shutdown()
    
    def perform_agent_tasks(self):
        """Выполняет задачи агента и управляет приложениями"""
        ...
        if not self.is_work_time():
            ...
            logging.debug("Outside work hours, inactive period...")
            return
        ...
        if self.is_break_time():
            ...
            logging.info("Break time, inactive period...")
            return
        ...
        if self.should_switch_app():
            ...
            # Пауза между приложениями (неактивное время)
            pause_time = random.randint(30, 120)
            logging.debug(f"Inactive period: {pause_time} seconds between applications")
        ...
        if self.current_app:
            ...
            # Пауза между активностями (неактивное время)
