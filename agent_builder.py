#!/usr/bin/env python3
"""
Система сборки агентов с конфигурацией из базы данных
Создает ELF файлы с помощью Nuitka
"""
...
class AgentBuilder:
    """Сборщик агентов с конфигурацией из БД"""
    ...
    def generate_agent_config(self, template_id: int) -> Optional[Dict[str, Any]]:
        ...
        config = {
            ...
            "role": behavior_template.get('role_name', 'User'),
            ...
            "operating_system": behavior_template.get('os_type', 'linux'),
    ...
    def generate_builtin_apps_code(self, used_apps: List[str]) -> str:
        ...
        # Полный список встроенных приложений (ваш существующий код)
        all_builtin_apps = {
            "Visual Studio Code": '''
    "Visual Studio Code": {
    ...
    def create_agent_source(self, config: Dict[str, Any], build_dir: Path) -> Path:
        """Создает исходный код агента с конфигурацией"""
...
class CompiledActivityAgent:
    """Скомпилированный агент активности"""
    ...
    def __init__(self):
        ...
        logging.info(f"Compiled agent started for user: {{self.config.get('username', 'unknown')}}")
    ...
    def get_available_applications(self):
        """Возвращает доступные приложения"""
        apps = {{}}
        
        # Добавляем встроенные приложения
    ...
    def convert_custom_app_config(self, db_config):
        """Конвертирует конфигурацию из БД в формат агента"""
...
'''
        ...
        logging.info(f"Agent source created: {agent_file}")
        return agent_file
    ...
    def update_build_record(self, template_id: int, build_status: str, binary_path: str = None, 
        ...
        # Получаем agent_id для template
    ...
    def build_agent(self, template_id: int, agent_name: str = None) -> Optional[Path]:
        ...
        try:
            ...
            # Генерируем имя агента
            if not agent_name:
                agent_name = f"agent_template_{template_id}_{int(time.time())}"
            
            logging.info(f"Building agent for template {template_id}")
            ...
            if binary_path and binary_path.exists():
                ...
                logging.info(f"Agent built successfully: {binary_path}")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python agent_builder.py <template_id> [agent_name]")
        sys.exit(1)
    ...
    if result:
        print(f"Agent built successfully: {result}")
        sys.exit(0)
    else:
